...
Graph node and edge IDs are versioned 50-byte logical values containing database, table, schema-version, and random
entity identity. They are stored in hidden generated columns and therefore survive heap relocation; raw page/slot
`RowId` values never become SQL graph identity. Registration accepts only empty tables with the exact trailing generated
columns and ordinary B-tree contracts: one unique identity index per node/edge table and non-unique outgoing/incoming
indexes over edge endpoints. Registered graph tables reject ordinary mutations. Graph handles generate identities,
validate live endpoints before effects, maintain heap plus all indexes through the existing compensation path, reject a
node delete while any edge references it, and reopen from catalog metadata. Directional lookup deduplicates a self-loop
for `Both`, orders by generation-safe physical edge identity, honors cancellation, and collects no more than the caller's
positive limit (hard maximum 100,000) before publishing results. Graph path algorithms, MATCH semantics, cycle policy,
path ordering, and multi-hop traversal remain executor concerns built over this bounded adjacency capability.

## Bootstrap binary format

Catalog format version 10 starts with the four bytes `43 41 54 30` (`CAT0`, the one-byte suffix for version 10), a little-endian 16-bit version, two zero reserved bytes, and 32-bit table, index, scalar-type, table-type, XML-collection, and assembly counts. XML collections and assemblies precede table/index/type records so typed XML and CLR declarations resolve shared identities while decoding. Table records encode database and schema before object name and end with optional system-versioning and graph metadata. Versions 6 through 9 remain readable; version 8 catalogs decode spatial indexes with the all-SRID policy used by that release, and version 9 adds exact spatial SRID without graph metadata. The next catalog publication upgrades an older form to version 10. Typed XML type records store only collection identity. Index records retain B-tree, JSON-path, namespace-bound XML, spatial SRID, vector, or full-text method metadata. The existing length-prefixed specialized-option vector carries full-text's nine versioned identity/resource fields; older readers reject the unknown method rather than misreading it. Exact-SRID spatial, full-text linguistic/resource options, and graph table/index ownership are catalog capabilities rather than lossy row filters.
